require([
  ...
  'use strict';
   $(document).ready(function(){
      ...
      var saveData = function (field) {
        ...
        if (absolute_url !== undefined) {
          ...
          .done(function(data) {
            ...
            var feedbackButton = wrapper.siblings('.sort-column').find('button .glyphicon');
            var statusClass = result.success === true ? 'has-success' : 'has-error';
            var statusIcon = result.success === true ? 'glyphicon-ok' : 'glyphicon-remove';
            wrapper.addClass(statusClass);
            feedbackButton.removeClass("glyphicon-option-vertical").addClass(statusIcon);
            setTimeout(function () {
              wrapper.removeClass(statusClass);
              feedbackButton.removeClass(statusIcon).addClass("glyphicon-option-vertical");
            }, 2000);
          })
          .fail(function(error) {
            console.error(error);
      ...
      //delete bookmark
      $(document).on('click', '.bookmarks-wrapper .bookmark-fields button.remove-bookmark', function (e) {
        e.preventDefault();
        removeBookmark($(e.target));
      });

      // change value in fields
      $(document).on('input', '.bookmarks-wrapper .bookmark-fields input[type="text"]', function (e) {
